...
**2026-07-30 — grrd's Puzzle source dive** (Scripts/puzzle.js, 1644 lines, MPL): borrow-worthy — (1) **orientation-aware difficulty presets**: easy/medium/hard choose rows×cols per portrait/landscape so pieces stay near-square, plus a persisted "gold" toggle that ~3×'s the counts under the same three buttons (kid mode / adult mode without a fourth control); (2) **background grid assist**: toggleable cell-outline rects on the board — structural help without spoiling the image (lighter than any ghost); (3) **drag clamping** (`dragBoundFunc` clamps to canvas) — pieces can never be flung out of reach; our pan/zoom world needs the equivalent (clamp to board + scatter ring); (4) **two sounds + persisted toggles**: click on join, ding on completion; all assist settings (rotate/sound/grid/image) remembered in localStorage. Superseded by our stack: its exif.js + iOS detectVerticalSquash workarounds (modern `createImageBitmap` handles this), Kinetic.js hit regions (Polygon hitArea), absolute-target snap for all pieces (we do cluster-merge + border frame-snap). Skip: per-image medal/level progression, 20-locale i18n, built-in image themes.
